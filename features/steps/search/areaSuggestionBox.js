...
Then(/^I should see the area list in the suggestion box$/, async () => {
    await expect(searchPage.suggestionBoxAreaContainer).toBeDisplayed();
});

Then(/^I click on the area list$/, async () => {
    await searchPage.suggestionBoxArea.click();
});

Then(/^I should see the area detail page$/, async () => {
    await expect(searchPage.areaListPage).toBeDisplayed();
